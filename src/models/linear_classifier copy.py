...
class FlexibleClassifier(nn.Module):
    def __init__(self, input_size: int, num_classes: int, hidden_layers=None, dropout_rate=0.6):
        super(FlexibleClassifier, self).__init__()
        layers = []
        current_input_size = (5 * 50) # + input_size# Adjusted input size
...
class ClassifierWithEncoder(pl.LightningModule):
    ...
    def forward(self, x):
        # x shape: torch.Size([32, 5, 448])
      
        # Extract the last candle data correctly given the shape
        # We take the last item in the sequence for all features, resulting in shape [batch_size, num_features]
        last_candle_data = x[:, :, -50:]  # Adjusted indexing to reflect correct dimensions
        last_candle_data = last_candle_data.reshape(last_candle_data.shape[0], -1)
        #print('last_candle_data shape: ', last_candle_data.shape)
        
        # get the last five candles
        # Process embeddings with the encoder
        # embeddings = self.encoder(x)
        # embeddings = embeddings.transpose(1, 2)
        # pooled_embeddings = self.avg_pool(embeddings).squeeze(-1)
        # # #print('pooled embeddings shape:', pooled_embeddings.shape)

        # combined_features = torch.cat([pooled_embeddings, last_candle_data], dim=1)
        combined_features = last_candle_data
