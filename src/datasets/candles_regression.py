...
def make_timeseries(
    ...
    dataset = RegressionCandleDataSet( 'data/binanceus/', freq="1d", training=training, transform=transform)
...
class RegressionCandleDataSet(Dataset):
    ...
    def preprocess_data(self):
        for file_path in self.data_files:
            ...
            for start_idx in range(len(df) - self.sequence_length):
                ...
                self.labels.append(self.calculate_class(sequence[-1][1], next_candle[1]))  # Close of last in sequence and High of next
    ...
    def __getitem__(self, idx):
        ...
        return data_tensor, torch.tensor(label, dtype=torch.float)
# Example usage
